...
class Taggable extends DataExtension
{
    ...
    */
    public function updateCMSFields(FieldList $fields)
    {
        ...
        } elseif (get_class($fields) == 'FieldSet' || get_class($fields) == 'FieldList') {
    ...
    public static function tags2Links($strTags)
    {
        ...
        if (!$tagsPageURL=self::getTagPageLink()) {
            throw new Exception('There is no page of type TagsPage in the site tree');
        }
    ...
    {
        if (method_exists($this->owner, 'getAssociatedImage')) {
            return $this->owner->getAssociatedImage();
        }
        if (method_exists($this->owner, 'getAddImage')) {
            return $this->owner->getAddImage();
        }
        if (method_exists($this->owner, 'Image')) {
    ...
    public function onBeforeWrite()
    {
        ...
        if (!$this->owner->Tags) {
            if (!empty($this->owner->MetaKeywords)) {
                ...
                } // sneakily increase title weighting
        ...
        if (!$this->owner->MetaKeywords) {
            if ($this->owner->Tags) {
                $this->owner->MetaKeywords = $this->owner->Tags;
            }
        }
